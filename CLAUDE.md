...
- **2026-07-20 — Event-store persona (schema del "CRM persone")**: la timeline del ciclo di vita operatore (assunzione → onboarding → certificazioni → promozioni → QA → uscita) è il vero pezzo mancante emerso dallo studio "abbiamo un CRM?". Scelto un **event-store append-only per persona** (invariante ladder §8.2: nessun cambiamento silenzioso; lo stato corrente è una PROIEZIONE degli eventi, non scritto a mano). Congelato lo **schema stabile** prima di scrivere il percorso di scrittura (decisione di sequenza di Nicholas): enum livelli L0-L6 (doppio binario L4a/L4b) + 16 tipi di evento in 5 categorie, in `src/lib/person-events.js` (PURO, testato, INERTE — nessuna scrittura KV finché l'ADR non è accettato). Namespace KV proposto `person:events:{id}` / `person:state:{id}` / `person:index`. ADR/spec in `docs/PERSON_EVENT_STORE.md` (stato PROPOSED): identità `personId` = decisione APERTA (proposta employeeId Infloww); backfill da dati esistenti (employee_profile, certifications.js, qa-reviews, cm:sup, disputes, action_center); scrittura + backfill + scheda persona 360 (`/admin/persone/[id]`) + progression engine = fasi successive dopo conferma. Confine: lo schema implementa i gate del ladder, NON ne fissa le soglie (restano decisione HR/board, §4.1/§11). Career ladder aggiornato a v0.6 §13.
