...
- **2026-07-20 — Retention creator (Livello 1 "CRM fan")**: studio su "abbiamo un CRM?" per persone e fan (simulazioni visive + ricerca settore). Esito fan: il "CRM fan" operativo **esiste già ed è Infloww** (whale list, fan scoring, tag, note — dove lavora il chatter); costruirne uno parallelo in HOC Pro duplica e apre due fronti (ToS OnlyFans: no API ufficiale, scraping/compilare DB vietato; GDPR Art.9: spesa su contenuti sessuali = dato speciale). Costruito invece il pezzo che Infloww NON dà: lente di **retention aggregata per creator** sul payout ledger — `src/lib/retention-calc.js` (funzioni pure) + `/api/admin/retention` + `/admin/retention` (`authorizeAll(SCORES_VIEW)`, sotto Comp & Ben). Contenuto: concentrazione whale (quota top 1/5/10/20%), coorti di retention (mese d'ingresso → ritorno), bande LTV (cold/small/regular/whale), whale dormienti (retention-risk). Fan **PSEUDONIMIZZATI** (`fanPseudonym`, mai nome/fid in output) — l'unica superficie con fan identificati resta `/admin/payout-tree` (evidenza dispute). **Nessuna azione sul fan qui by design**: l'azione (messaggiare/riattivare) vive in Infloww/OF. Status vendita realizzata = `st === "done"` (verificato su dati reali: ledger usa done/undo/loading, non complete). Misurato lug 2026 (Gaja ITA, 2066 fan): top 10% fan = 62,7% dei ricavi, 97 whale = 46,7%, refund 1,6%. Livello 2 (alert dormienza → CM che riattiva in Infloww) GATED su un processo retention con owner — non costruito. "CRM persone" è track separato (event-store sul career ladder).
