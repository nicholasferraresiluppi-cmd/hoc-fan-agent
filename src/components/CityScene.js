...
function mountCity(root,RAW,THREE,OrbitControls){
  ...
  root.innerHTML="<div class=\"intro\" id=\"intro\"><b>La citt\u00e0</b><i></i></div>\n<canvas id=\"c\" aria-label=\"La citt\u00e0 dell'azienda: un palazzo per ogni progetto, un piano per ogni area\"></canvas>\n<div class=\"top\"><b>La citt\u00e0</b><div class=\"r\"><span class=\"tag\" id=\"tag\">Dati di ClickUp \u00b7 aree stimate</span><button class=\"pill\" id=\"snd\" aria-pressed=\"false\">Suono spento</button></div></div>\n<div class=\"labels\" id=\"labels\"></div>\n<div class=\"hero\" id=\"hero\">\n  <h1 id=\"h1\">La citt\u00e0.<br><em>Una sola citt\u00e0.</em></h1>\n  <p id=\"line\">Ogni palazzo \u00e8 un progetto, ogni piano \u00e8 un'area. Tocca un palazzo per entrarci, o scegli un'area qui sotto per vederla in tutta la citt\u00e0.</p>\n  <div class=\"stats\"><div><b id=\"s1\">0</b><span>progetti</span></div><div><b id=\"s2\">0</b><span>aree in ritardo</span></div><div><b id=\"s3\">0</b><span>aree ferme</span></div></div>\n</div>\n<nav class=\"chips\" id=\"chips\" aria-label=\"Guarda un'area in tutta la citt\u00e0\"></nav>\n<aside class=\"panel\" id=\"panel\" aria-live=\"polite\"><button class=\"x\" id=\"close\">Chiudi</button><div id=\"pb\"></div></aside>";
...
const COL={ok:new THREE.Color(0x7FE0B8),wait:new THREE.Color(0xFFB54A),stop:new THREE.Color(0x3A3C44),none:new THREE.Color(0x1E1F25)};
...
cells.forEach(([c,r])=>{const isHQ=(c===2&&r===HQR);if(!isHQ&&pi>=projects.length)return;const data=isHQ?HQ:projects[pi++];const x=(c-2)*CW,z=(r-(ROWS-1)/2)*CD;
  ...
  const w=isHQ?2.5:1.8,d=isHQ?1.6:1.15,t=isHQ?.1:.08,gap=isHQ?.72:.5,n=data.areas.length;
...
// ---------- UI ----------
{const n=towers.length;const late=towers.reduce((a,T)=>a+T.data.areas.filter(x=>x.s==='wait').length,0);$id('h1').innerHTML=`${(NUMW[n]||n).replace(/^./,c=>c.toUpperCase())} palazzi.<br><em>${late?`${late} aree aspettano qualcosa.`:'Una sola città.'}</em>`}
...
function setFilter(name){filter=name;chips.querySelectorAll('button').forEach(b=>b.setAttribute('aria-pressed',(+b.dataset.k<0?null:AREAS[+b.dataset.k])===name));
  ...
  const c={ok:0,wait:0,stop:0,none:0};towers.forEach(T=>{const a=areaOf(T,name);if(a)c[a.s]++});
  lineEl.textContent=`${name} in tutta la città: ${c.ok} in movimento, ${c.wait} con qualcosa in ritardo, ${c.stop} ${c.stop===1?'ferma':'ferme'}${c.none?`, ${c.none} senza attività`:''}.`;chime(3,true)}
...
// intro
let t0=performance.now()+(reduce?-20000:2400),shown=false;const intro=$id('intro');
if(reduce){intro.classList.add('gone')}else{timers.push(setTimeout(()=>intro.classList.add('go'),120),setTimeout(()=>intro.classList.add('gone'),2200))}
...
function loop(){const dt=Math.min(.05,clock.getDelta()),now=performance.now(),t=(now-t0)/1000;
  ...
  towers.forEach(T=>{
    ...
    T.slabs.forEach(S=>{
      ...
      const kk=Math.max(0,lit)*dim*pulse*(hoverT===T&&!selT?1.25:1);
