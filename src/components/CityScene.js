...
controls.enableDamping=true;controls.dampingFactor=.06;controls.enablePan=false;controls.minDistance=9;controls.maxDistance=40;controls.minPolarAngle=.7;controls.maxPolarAngle=1.45;controls.rotateSpeed=.6;
...
const houses=HOUSES.map((h,hi)=>{
  const ang=0;const pos=new THREE.Vector3(-hi*12.5,0,-hi*7.5);
  ...
  h.a.forEach((a,i)=>{const g=new THREE.Group();
    const metal=new THREE.Mesh(metalGeo,metalMat);metal.position.y=-.17;metal.castShadow=true;metal.receiveShadow=true;g.add(metal);
    const glass=new THREE.Mesh(glassGeo,glassMat);glass.castShadow=true;glass.userData={h:hi,i};g.add(glass);
  ...
  const plinth=new THREE.Mesh(plinthGeo,plinthMat);plinth.position.y=.11;plinth.receiveShadow=true;plinth.castShadow=true;root.add(plinth);
  const band=new THREE.Mesh(new THREE.TorusGeometry(2.72,.018,16,160),goldMat);band.rotation.x=Math.PI/2;band.position.y=.225;root.add(band);
  const rod=new THREE.Mesh(new THREE.CylinderGeometry(.045,.045,1,24),goldMat);root.add(rod);
  const cap=new THREE.Mesh(new THREE.SphereGeometry(.11,32,32),goldMat);root.add(cap);
  ...
  return {h,hi,root,sculpt,layers,rod,cap,pos,ang,N,openT:0,vis:hi===0?1:0,t0:0};
});
...
// stato
let open=0,sel=null,hover=null,T0=performance.now();
...
function go(i){if(i===cur&&shown)return;deselect();cur=(i+houses.length)%houses.length;houses.forEach(H=>H.vis=H.hi===cur?1:0);houses[cur].t0=performance.now();
...
function frame(){const H=houses[cur];const m=VW()<760;const d=m?25:13.2;
...
on(cv,'pointermove',e=>{mouse.set(...toXY(e));ray.setFromCamera(mouse,cam);const hit=ray.intersectObjects(pick)[0];hover=hit&&hit.object.userData.h===cur?hit.object.userData.i:null;cv.style.cursor=hit?'pointer':'grab'});
on(cv,'pointerdown',e=>downAt=[e.clientX,e.clientY]);
on(cv,'pointerup',e=>{if(!downAt)return;const moved=Math.hypot(e.clientX-downAt[0],e.clientY-downAt[1]);downAt=null;if(moved>6)return;mouse.set(...toXY(e));ray.setFromCamera(mouse,cam);const hit=ray.intersectObjects(pick)[0];if(hit){if(hit.object.userData.h!==cur)go(hit.object.userData.h);else select(hit.object.userData.i)}else if(sel!=null)deselect()});
...
on(document,'keydown',e=>{if(e.key==='Escape')deselect();if(sel!=null&&e.key==='ArrowDown')select((sel+1)%houses[cur].N);if(sel!=null&&e.key==='ArrowUp')select((sel+houses[cur].N-1)%houses[cur].N);if(sel==null&&e.key==='ArrowRight')go(cur+1);if(sel==null&&e.key==='ArrowLeft')go(cur-1)});
const ob=$id('open');ob.onclick=()=>{open=open?0:1;ob.textContent=open?'Richiudi la struttura':'Apri la struttura';frame()};

go(0);houses[0].t0=performance.now()+(reduce?-1e5:2600);
...
function loop(){
  ...
  houses.forEach(X=>{const t=(now-X.t0)/1000;const isCur=X.hi===cur;
    if(!reduce&&sel==null&&isCur)X.sculpt.rotation.y=Math.sin(t*.18)*.32;else X.sculpt.rotation.y+=(0-X.sculpt.rotation.y)*Math.min(1,dt*2);
    X.layers.forEach((L,i)=>{
      const delay=.25+(X.N-1-i)*.16;const p=reduce||!isCur?1:Math.min(1,Math.max(0,(t-delay)/1.1));const e=ease(p);
      ...
      const lit=isCur?(p>=1?Math.min(1,(t-delay-1.1)*1.6):0):1;
      const dim=(isCur?((sel!=null&&sel!==i)?.25:1):.22);
      let pulse=1;if(L.a.s==='wait'&&!reduce)pulse=.55+.45*(.5+.5*Math.sin((now/1000)*2.2+i));
      const k=Math.max(0,lit)*dim*pulse;
      if(L.a.s!=='stop'){L.sp.material.opacity=.7*k;L.coreMat.color.copy(COL[L.a.s]).multiplyScalar(.35+.65*k)}
    });
  ...
  key.position.set(H.pos.x+6,10,H.pos.z+5);key.target.position.copy(H.pos);
  if(!shown&&(now-houses[cur].t0)/1000>.25+(H.N-1)*.16+1.8){shown=true;root.classList.add('show');const vv=heroFor(H.h);countUp(vv)}
  ...
  houses.forEach((X,xi)=>{const el=bldEls[xi];if(X.hi===cur){el.style.opacity=0;el.style.pointerEvents='none';return}
    v.copy(X.pos);v.y=baseY(X,0)+1.1;const pr=v.clone().project(cam);const on=pr.z<1&&Math.abs(pr.x)<1.1&&Math.abs(pr.y)<1.1;
    el.style.opacity=on&&sel==null?.9:0;el.style.pointerEvents=on&&sel==null?'auto':'none';el.style.transform=`translate(${(pr.x*.5+.5)*VW()}px,${(-pr.y*.5+.5)*VH()}px) translate(-50%,-100%)`});
