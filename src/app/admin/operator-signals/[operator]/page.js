...
function MomentCard({ m, tone, twin, onOpenTwin }) {
          ...
          {data.totals?.wins === 0 && data.totals?.losses === 0 ? (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(420px, 1fr))", gap: 18, alignItems: "start" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {data.wins.map((m) => (
                    <div key={m.key} id={`moment-${m.key}`} style={highlight === m.key ? { outline: `2px solid ${CP.accent}`, borderRadius: 12 } : undefined}>
                      <MomentCard m={m} tone="win" />
                    </div>
                  ))}
                  {data.wins.length === 0 && <div style={{ fontSize: 13, color: CP.textMuted }}>Nessuna chiusura ≥ $100 nel periodo.</div>}
                </div>
              </section>
              ...
              <section>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {data.losses.map((m) => (
                    <MomentCard key={m.key} m={m} tone="loss" twin={m.twin_key ? winByKey.get(m.twin_key) : null} onOpenTwin={openTwin} />
                  ))}
