...
const SCOPE_COLOR = { own: "#10B981", team: "#3B82F6", all: "#F59E0B" };

const PALETTE = ["#64748B", "#EF4444", "#F59E0B", "#10B981", "#3B82F6", "#8B5CF6", "#EC4899", "#14B8A6"];
const EMOJIS = ["🎖️", "🛡️", "⚡", "🎯", "🚀", "🔧", "📊", "🧭", "🏆", "🕹️", "👔", "🧩"];

function emptyRole() {
  return { id: "", name: "", emoji: "🎖️", color: "#64748B", description: "", capabilities: {} };
}
...
export default function CustomRolesPage() {
  ...
  return (
    <div style={{ background: "#0D0D0D", minHeight: "100vh", color: "#fff", padding: "2rem" }}>
      ...
      {/* Editor */}
      <div style={{ border: "1px solid #333", borderRadius: 10, padding: "1rem", background: "#111", marginBottom: "2rem" }}>
        <h3 style={{ marginTop: 0 }}>{draft.id?.startsWith("c:") ? `Modifica ${draft.id}` : "Nuovo ruolo"}</h3>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.75rem", marginBottom: "0.75rem" }}>
          <label>
            <div style={{ fontSize: "0.72rem", color: "#888" }}>ID (snake_case, univoco)</div>
            <input
              ...
              style={{ width: "100%", padding: "0.45rem", background: "#0a0a0a", color: "#fff", border: "1px solid #333", borderRadius: 6 }}
            />
          </label>
          <label>
            <div style={{ fontSize: "0.72rem", color: "#888" }}>Nome</div>
            <input
              value={draft.name}
        ...
        <label style={{ display: "block", marginBottom: "0.75rem" }}>
          <div style={{ fontSize: "0.72rem", color: "#888" }}>Descrizione</div>
          <input
        ...
        <div style={{ display: "flex", gap: "0.5rem", alignItems: "center", marginBottom: "0.75rem", flexWrap: "wrap" }}>
          <span style={{ fontSize: "0.72rem", color: "#888" }}>Emoji:</span>
          {EMOJIS.map((e) => (
            <button key={e} onClick={() => setDraft({ ...draft, emoji: e })}
              style={{ padding: "0.25rem 0.45rem", background: draft.emoji === e ? "#333" : "transparent", border: "1px solid #333", borderRadius: 6, cursor: "pointer" }}>
              {e}
            </button>
          ))}
          <span style={{ fontSize: "0.72rem", color: "#888", marginLeft: "1rem" }}>Colore:</span>
          {PALETTE.map((c) => (
            <button key={c} onClick={() => setDraft({ ...draft, color: c })}
              style={{ width: 22, height: 22, background: c, border: draft.color === c ? "2px solid #fff" : "1px solid #333", borderRadius: 999, cursor: "pointer" }} />
          ))}
        </div>
        ...
        <div style={{ border: "1px solid #222", borderRadius: 8, overflow: "hidden" }}>
          {caps.map((cap) => {
            ...
            return (
              <div key={cap} style={{ display: "flex", alignItems: "center", gap: "0.75rem", padding: "0.5rem 0.75rem", borderBottom: "1px solid #222", background: active ? "#101820" : "transparent" }}>
                <input type="checkbox" checked={active} onChange={() => toggleCap(cap)} />
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: 600 }}>{CAP_LABELS[cap] || cap}</div>
                  <div style={{ fontSize: "0.68rem", color: "#666" }}>{cap}</div>
                </div>
                  ...
                  {scopes.map((s) => (
                    <button
                      ...
                      style={{
                        ...
                        color: scope === s ? "#0D0D0D" : active ? SCOPE_COLOR[s] : "#444",
                        border: `1px solid ${active ? SCOPE_COLOR[s] : "#333"}`,
                        borderRadius: 4,
        ...
        <div style={{ display: "flex", gap: "0.5rem", marginTop: "1rem" }}>
          <button onClick={save} disabled={busy}
            style={{ padding: "0.5rem 1rem", background: "#10B981", color: "#0D0D0D", border: 0, borderRadius: 6, fontWeight: 800, cursor: "pointer" }}>
            {busy ? "Salvo…" : "Salva ruolo"}
          </button>
          <button onClick={resetDraft}
            style={{ padding: "0.5rem 1rem", background: "transparent", color: "#aaa", border: "1px solid #333", borderRadius: 6, cursor: "pointer" }}>
            Reset
          </button>
        </div>
      </div>
      ...
      {!loading && (data?.roles || []).length === 0 && <p style={{ color: "#666" }}>Nessun ruolo custom. Creane uno sopra.</p>}
          ...
          <div key={r.id} style={{ border: `1px solid ${r.color}55`, borderRadius: 10, padding: "0.75rem", background: `${r.color}10` }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "start" }}>
              <div>
                ...
                <div style={{ fontSize: "0.7rem", color: "#888" }}>{r.id}</div>
              </div>
              <div style={{ display: "flex", gap: "0.25rem" }}>
                <button onClick={() => editRole(r)} style={{ padding: "0.2rem 0.5rem", background: "transparent", color: "#aaa", border: "1px solid #333", borderRadius: 4, cursor: "pointer", fontSize: "0.7rem" }}>✎</button>
                <button onClick={() => remove(r.id)} style={{ padding: "0.2rem 0.5rem", background: "transparent", color: "#EF4444", border: "1px solid #EF4444", borderRadius: 4, cursor: "pointer", fontSize: "0.7rem" }}>×</button>
              </div>
            </div>
            {r.description && <div style={{ fontSize: "0.75rem", color: "#aaa", marginTop: "0.35rem" }}>{r.description}</div>}
