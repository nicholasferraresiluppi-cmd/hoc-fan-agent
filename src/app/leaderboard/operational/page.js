...
function generateWeeklyOptions(count = 16) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    // Calcola date inizio/fine settimana ISO
  ...
  // dedup by value, mantenendo l'ordine
  const seen = new Set();
  return out.filter((o) => {
...
function Avatar({ name, size = 40, large = false }) {
  return (
    <div style={{
      ...
      borderRadius: "50%",
      background: large
        ? `linear-gradient(135deg, ${COLORS.champagne} 0%, ${COLORS.charcoal} 100%)`
        : COLORS.charcoal,
      ...
      boxShadow: large
        ? `0 0 0 2px ${COLORS.champagne}, 0 8px 24px rgba(212,175,122,0.25)`
        : "none",
...
function MiniBar({ value, color }) {
  return (
    <div style={{
      height: 4, width: "100%", maxWidth: 90,
      background: COLORS.charcoal, borderRadius: 999, overflow: "hidden",
    }}>
...
function StatCard({ label, value, sub, color }) {
  return (
    <div style={{
      ...
      {sub && (
        <div style={{ fontSize: 11, color: COLORS.mist, marginTop: 4 }}>{sub}</div>
      )}
    </div>
  );
}
...
function KpiVsGroup({ value, mean, formatter, label }) {
  // Mostra valore operatore + media Group sotto
  ...
  return (
    <div title={`${label}: media Group ${formatter(mean)}`}>
      <div style={{
        ...
        color: isAbove ? "#3FB97E" : COLORS.alabaster,
      }}>{formatter(value)}</div>
      {mean != null && mean > 0 && (
        <div style={{
          fontFamily: FONTS.mono, fontSize: 10, color: COLORS.mist, marginTop: 1,
        }}>Ø {formatter(mean)}</div>
      )}
    </div>
  );
}
    ...
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 18, position: "relative" }}>
        ...
        <div>
          ...
          <div style={{
            color: COLORS.champagne, fontSize: 12,
      ...
      <div style={{ textAlign: "right", position: "relative" }}>
        ...
        <div style={{
          fontFamily: FONTS.mono, fontWeight: 700, fontSize: 56,
          lineHeight: 1, color: tierColor,
        }}>{op.score?.toFixed(1)}</div>
        <div style={{
          fontFamily: FONTS.mono, color: COLORS.mist, fontSize: 13, marginTop: 2,
        }}>/ 100</div>
      </div>
      <div style={{
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", marginBottom: 8 }}>
        <div style={{ fontFamily: FONTS.mono, fontWeight: 700, fontSize: 24, color: tierColor }}>
          {op.score?.toFixed(1)}
        </div>
        <TierBadge tier={op.tier} />
      </div>
...
function StreamRow({ op, groupMeans }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ color: COLORS.fog, fontSize: 12 }}>{op.group}</div>
...
export default function OperationalLeaderboardPage() {
  ...
  // Default al primo periodo della lista quando cambia tipo
  useEffect(() => {
  ...
  const styles = {
    ...
    title: {
      fontFamily: FONTS.display, fontSize: 32, margin: "0 0 6px 0",
  ...
  return (
    <div style={styles.page}>
      <div style={styles.container}>
        ...
        {/* States */}
        {isLoading && !data && (
          <p style={{ color: COLORS.fog }}>Caricamento…</p>
        )}
        {error && (
        ...
        {data && !data.error && (
          <div style={styles.summary}>
            ...
            <StatCard
              label="Score medio"
              value={`${data.avg_score?.toFixed(1)} / 100`}
            />
            <StatCard
              label="Tier Elite"
        ...
        {/* Hero #1 */}
        {heroOp && (
          <HeroCard op={heroOp} groupMeans={groupAverages[heroOp.group]} />
        )}
